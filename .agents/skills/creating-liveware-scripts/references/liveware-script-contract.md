...
Automatic Node analysis never proves a ready adapter from JavaScript source. A package `liveware` or `start` script is always ambiguous and must report package and available entry evidence. Render only from a user-confirmed exact argv and default port, plus confirmation that the command consumes exported `PORT` or uses a standalone `{port}` argument. This avoids guessing JavaScript control flow.
...
Inspect lifecycle evidence before returning any Python, Node, or static automatic candidate. Evidence includes a noncanonical `liveware/scripts/start.sh`, Docker or Compose configuration, supervisor or s6 configuration, root or `liveware` service units, common PM2 configuration, exact launcher names, and reference statements that explicitly connect a known manager to service ownership or execution. Launcher names use exact action tokens (`start`, `run`, `launch`, `launcher`, `serve`, or `server`) plus `liveware`, `server`, `service`, or `app`; `scripts/start.*` is always explicit. Ordinary examples and unrelated prose are not lifecycle evidence. An exact generated start script is exempt only when its strict manifest decodes for the current target and skill and every byte equals a fresh canonical render; plausible or tampered markers are not proof. Treat all other custom service managers, conflicting entrypoints, and unknown ports as ambiguous until the user confirms the interface. Never convert a command into `shell=True` or an unquoted shell string.
