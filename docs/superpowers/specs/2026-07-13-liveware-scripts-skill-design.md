...
5. 分析状态为 `ambiguous` 或 `blocked` 时停止并向用户澄清。
6. 写入后执行允许的静态检查。
...
Manifest schema 只允许顶层 `schema_version`、`status`、`target_root`、`skill_name`、可选 `display_name`、`adapter`、`static_dir`、`evidence` 与 `issues`。Adapter、readiness、log 和 evidence item 同样采用封闭对象 schema；任何层级的缺失必填属性或额外属性均使 manifest 无效。`schema_version` 与动态 `default_port` 使用精确整数类型，不能以布尔值或浮点数替代。
